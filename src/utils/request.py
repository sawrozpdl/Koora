...
def api_call(*args, **kwargs):
    ...
    headers = {
        ...
        'Token' : str(kwargs['request'].user.id)
    }
    ...
    return getattr(requests, kwargs.get('method', 'get'))(url = kwargs['request'].build_absolute_uri(generate_url_for(reverse_for, kwargs=reverse_kwargs, query=reverse_params)), headers=headers, cookies=cookies, data=data, files=files)
